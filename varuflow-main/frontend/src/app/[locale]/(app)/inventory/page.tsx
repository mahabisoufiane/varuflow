...
import { EmptyInventory } from "@/components/illustrations";
...
interface StockLevel {
  ...
  product: {
    ...
    unit: string;
    category?: string;
    // v38 (Item 16) — auto-reorder fields surfaced by ProductOut so
    // the badge can render without a second fetch. Optional so the
    // row still works if the backend (or a stale cached response)
    // omits them.
...
export default function InventoryPage() {
  ...
  const NAV_TILES = [
    ...
    { href: "/inventory/labels",          icon: Printer,      label: "Labels",          sub: "Print barcodes & QR"          },
    { href: "/inventory/transfers",       icon: Truck,        label: "Transfers",       sub: "Move stock between warehouses" },
  ];
      ...
      <div className="vf-section">
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            title={search ? "No results found" : "No stock data yet"}
            description={search ? "Try a different search term" : "Add products to see stock levels here"}
          />
        ) : (
                ...
                <tr key={s.id} className={cx("vf-row", styles.tableRow)}>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <AutoReorderBadge
                        autoReorderEnabled={s.product.auto_reorder_enabled}
                        preferredSupplierId={s.product.preferred_supplier_id}
