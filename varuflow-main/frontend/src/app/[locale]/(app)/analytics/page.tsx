...
const STATUS_COLORS: Record<string, string> = {
  PAID: "#059669", SENT: "#2563EB", OVERDUE: "#DC2626", DRAFT: "#94a3b8",
};
...
export default function AnalyticsPage() {
  ...
  async function handleExport() {
    setExporting(true);
    try {
      // Go through api.downloadBlob so 401s trigger silent session-refresh
      // retry and non-OK responses surface a toast instead of silently
      // saving an error-payload file named "analytics-....pdf" that the
      // user then tries to open.
      await api.downloadBlob(
        `/api/analytics/export/pdf?from_date=${fromDate}&to_date=${toDate}`,
        `analytics-${fromDate}-${toDate}.pdf`,
      );
    } catch {
      ...
      <div className="vf-section p-6">
        ...
        <ResponsiveContainer width="100%" height={240}>
          <AreaChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="Invoiced"  stroke="#2563EB" fill="url(#gInvoiced)"  strokeWidth={2} dot={false} />
        ...
        <div className="vf-section p-6">
          ...
          <ResponsiveContainer width="100%" height={Math.max(200, barData.length * 38)}>
            <BarChart data={barData} layout="vertical" margin={{ top: 0, right: 48, left: 8, bottom: 0 }}>
              ...
              />
              <Bar dataKey="Revenue" fill="#2563EB" radius={[0, 6, 6, 0]} maxBarSize={20} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="vf-section p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: "Products",    value: data.inventory.total_products,  color: "#2563EB" },
