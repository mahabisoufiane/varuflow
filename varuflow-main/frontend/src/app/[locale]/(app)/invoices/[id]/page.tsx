...
export default function InvoiceDetailPage() {
  ...
  async function handlePeppolExport() {
    if (!invoice) return;
    setPeppolExporting(true); setError(null);
    try {
      await api.downloadBlob(
        `/api/einvoice/peppol/${invoice.id}`,
        `varuflow-invoice-${invoice.invoice_number}.xml`,
        "POST",
      );
    } catch (e: any) { setError(e.message); } finally { setPeppolExporting(false); }
  }

  async function handleSendDepositReceipt() {
    if (!invoice) return;
    setReceiptSending(true); setSendMsg(null); setError(null);
    try {
      const res = await api.post<{ status: string; to?: string; reason?: string }>(
        `/api/invoicing/invoices/${invoice.id}/deposit-receipt`, {}
      );
      setSendMsg(res.status === "sent" ? `Receipt sent to ${res.to}` : res.reason ?? "Skipped");
    } catch (e: any) { setError(e.message); } finally { setReceiptSending(false); }
  }
      ...
      <div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" disabled={peppolExporting} onClick={handlePeppolExport}>
              {peppolExporting ? "…" : tInv("exportPeppolXml")}
            </Button>
            ...
            {invoice.invoice_type === "deposit" && invoice.status === "PAID" && invoice.customer.email && (
              <Button variant="outline" size="sm" disabled={receiptSending} onClick={handleSendDepositReceipt}>
                <Mail className="mr-1.5 h-3.5 w-3.5" />{receiptSending ? "Sending…" : "Send deposit receipt"}
              </Button>
            )}
      ...
      <div className="rounded-xl border bg-white overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tfoot className="border-t bg-gray-50 text-sm">
            ...
            {invoice.invoice_type === "final" && invoice.deposit_amount && Number(invoice.deposit_amount) > 0 && (() => {
              const dep = Number(invoice.deposit_amount);
              const due = Math.max(0, Number(invoice.total_sek) - dep);
              return (
                <>
