...
async function peekPosSales(): Promise<QueueEntry[]> {
  const db = await idbOpen(QUEUE_DB, 1);
  const sales: QueueEntry[] = [];
  return new Promise((resolve, reject) => {
    ...
    req.onsuccess = () => {
      ...
      if (!cursor) { resolve(sales); return; }
...
async function removePosSales(ids: number[]): Promise<void> {
...
async function replaySales(): Promise<void> {
  ...
  // Peek without deleting — entries stay in IDB until terminal response.
  const sales = await peekPosSales();
  ...
  try {
    ...
    if (res.ok || (res.status >= 400 && res.status < 500)) {
      // Terminal response — remove from IDB
      await removePosSales(sales.map((e) => e.id!));
    }
    // 5xx — leave in IDB for next sync event
  } catch {
    // Network error — leave in IDB for next sync event
  }
}
