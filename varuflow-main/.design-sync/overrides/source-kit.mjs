// forked from design-sync lib/source-kit.mjs — filter .module.scss imports from synth-entry barrel
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const SCSS_MODULE_RX = /from\s+['"][^'"]+\.module\.scss['"]/;
    // `export *` skips default exports — detect named default-export components
    // and emit an explicit `export { default as Name }` line for each so they
    // land on window.<GLOBAL>.<Name> alongside the named-export components.
    const DEFAULT_NAMED_FN_RX = /export\s+default\s+(?:async\s+)?(?:function|class)\s+([A-Z][A-Za-z0-9]*)/;
    const comps = srcFiles.filter((p) => {
      if (!SRC_IMPL_RX.test(p) || NON_IMPL_RX.test(p)) return false;
      if (SCSS_MODULE_RX.test(readText(p))) {
        console.error(`  [OVERRIDE] skipping ${p} from synth-entry barrel — imports .module.scss`);
        return false;
      }
      return true;
    });
    entry = join(OUT, '.pkg-entry.mjs');
    const barrel = comps.map((p) => {
      const text = readText(p);
      const m = DEFAULT_NAMED_FN_RX.exec(text);
      if (m) return `export * from ${JSON.stringify(p)};\nexport { default as ${m[1]} } from ${JSON.stringify(p)};`;
      return `export * from ${JSON.stringify(p)};`;
    }).join('\n') + '\n';
    writeFileSync(entry, barrel);
    synthEntry = true;
    console.error(
