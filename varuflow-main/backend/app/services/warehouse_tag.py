"""Pure helpers for warehouse tags (Item 84).
...
def normalize_name(raw: object) -> str:
    ...
    # Collapse internal whitespace to single spaces so "  Cold-chain "
    # and "Cold-chain" produce the same canonical name.
...
def assert_under_limit(*, current_count: int) -> None:
    ...
    if current_count >= MAX_TAGS_PER_WAREHOUSE:
        raise ValueError(
            f"warehouse already has {MAX_TAGS_PER_WAREHOUSE} tags "
            "(limit reached)"
        )
