"""Pure helpers for product tags (Item 81).
...
def normalize_name(raw: object) -> str:
    ...
    # Collapse internal whitespace to single spaces so "  Seasonal  "
    # and "Seasonal" produce the same canonical name.
...
def assert_under_limit(*, current_count: int) -> None:
    ...
    if current_count >= MAX_TAGS_PER_PRODUCT:
        raise ValueError(
            f"product already has {MAX_TAGS_PER_PRODUCT} tags "
            "(limit reached)"
        )
