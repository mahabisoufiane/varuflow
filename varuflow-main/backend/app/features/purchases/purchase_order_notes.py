"""Purchase order notes router (Item 89).

Endpoints under ``/api/purchase-order-notes``:

    GET    ""                    list notes (filter by purchase_order_id)
    POST   ""                    create a note
    GET    /{note_id}             detail
...
author — backdating someone else's procurement context would
silently rewrite audit history. Deletion is slightly more
...
from .purchase_order_note import PurchaseOrderNote
...
async def _load(
    ...
    row = await db.get(PurchaseOrderNote, note_id)
    if row is None or row.org_id != org_id:
        raise HTTPException(status_code=404, detail="Note not found")
    return row


def _to_out(row: PurchaseOrderNote) -> NoteOut:
    return NoteOut(
        id=row.id,
        ...
        mentions=svc_89.extract_mentions(row.body),
...
async def _count_pinned(
    ...
    stmt = select(func.count(PurchaseOrderNote.id)).where(
        PurchaseOrderNote.org_id == org_id,
        PurchaseOrderNote.purchase_order_id == purchase_order_id,
        PurchaseOrderNote.is_pinned.is_(True),
    )
    if exclude_id is not None:
        stmt = stmt.where(PurchaseOrderNote.id != exclude_id)
...
async def list_notes(
    ...
    _user, member = ctx
    stmt = select(PurchaseOrderNote).where(
        PurchaseOrderNote.org_id == member.org_id,
    )
    ...
    if pinned_only:
        stmt = stmt.where(PurchaseOrderNote.is_pinned.is_(True))
    # Pinned bubble up; ties break by newest-first.
    stmt = stmt.order_by(
        PurchaseOrderNote.is_pinned.desc(),
        PurchaseOrderNote.created_at.desc(),
    ).limit(limit)
...
async def create_note(
    ...
    await _assert_po_belongs(
        db, purchase_order_id=body.purchase_order_id, org_id=member.org_id,
    )
    try:
        text = svc_89.validate_body(body.body)
    ...
    if body.is_pinned:
        ...
        try:
            svc_89.assert_pin_limit(current_pinned=existing)
        except ValueError as e:
            raise HTTPException(status_code=400, detail=str(e))

    row = PurchaseOrderNote(
        org_id=member.org_id,
        purchase_order_id=body.purchase_order_id,
...
async def update_note(
    ...
    await log_action(
        db,
        action="purchase_order_note.updated",
        org_id=member.org_id,
        actor_user_id=user["user_id"],
        target_type="purchase_order_note",
        target_id=str(row.id),
        request=request,
        extra={"mentions": svc_89.extract_mentions(text)},
    )
    await db.commit()
    await db.refresh(row)
    return _to_out(row)
...
async def delete_note(
    ...
    await log_action(
        db,
        action="purchase_order_note.deleted",
        org_id=member.org_id,
        actor_user_id=user["user_id"],
        target_type="purchase_order_note",
...
async def unpin_note(
    ...
    await log_action(
        db,
        action="purchase_order_note.unpinned",
        org_id=member.org_id,
        actor_user_id=user["user_id"],
        target_type="purchase_order_note",
        target_id=str(row.id),
