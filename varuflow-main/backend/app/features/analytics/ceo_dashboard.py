...
from app.features.auth.organization import OrgPlan, OrgRole

log = logging.getLogger(__name__)
# CEO dashboards expose company-wide financial KPIs — manager-level (ADMIN+).
router = APIRouter(prefix="/api/ceo", tags=["ceo"], dependencies=[Depends(require_module("finance")), Depends(require_role(OrgRole.ADMIN))])
