...
# VAT returns are manager-level finance data — gate at ADMIN.
router = APIRouter(prefix="/api/accounting", tags=["vat"], dependencies=[Depends(require_module("finance")), Depends(require_role(OrgRole.ADMIN))])
...
async def list_vat_periods(
    ...
    try:
        from .vat_period import VatPeriod
        org_id = _org(ctx)
