"""Customer notes router (Item 71).

Endpoints under ``/api/customer-notes``:

    GET    ""                   list notes (filter by customer_id)
    POST   ""                   create a note
    GET    /{note_id}            detail
...
author — backdating someone else's CRM history would silently
...
async def _load(
    ...
    row = await db.get(CustomerNote, note_id)
    if row is None or row.org_id != org_id:
        raise HTTPException(status_code=404, detail="Note not found")
    return row


def _to_out(row: CustomerNote) -> NoteOut:
    return NoteOut(
        id=row.id,
        ...
        mentions=svc_71.extract_mentions(row.body),
...
async def _count_pinned(
    ...
    stmt = select(func.count(CustomerNote.id)).where(
        CustomerNote.org_id == org_id,
        CustomerNote.customer_id == customer_id,
        CustomerNote.is_pinned.is_(True),
    )
    if exclude_id is not None:
        stmt = stmt.where(CustomerNote.id != exclude_id)
...
async def list_notes(
    ...
    _user, member = ctx
    stmt = select(CustomerNote).where(CustomerNote.org_id == member.org_id)
    if customer_id is not None:
        stmt = stmt.where(CustomerNote.customer_id == customer_id)
    if pinned_only:
        stmt = stmt.where(CustomerNote.is_pinned.is_(True))
...
async def create_note(
    ...
    try:
        text = svc_71.validate_body(body.body)
    ...
    if body.is_pinned:
        ...
        try:
            svc_71.assert_pin_limit(current_pinned=existing)
        except ValueError as e:
            raise HTTPException(status_code=400, detail=str(e))
    ...
    await log_action(
        ...
        extra={
            ...
            "mentions":    svc_71.extract_mentions(text),
        },
    )
    await db.commit()
...
async def update_note(
    ...
    await log_action(
        ...
        target_id=str(row.id),
        request=request,
        extra={"mentions": svc_71.extract_mentions(text)},
    )
    await db.commit()
    await db.refresh(row)
    return _to_out(row)
