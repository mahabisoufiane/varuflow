...
from .shift import Shift, ShiftPunch, RosterPublication
from .shift_swap import ShiftSwapRequest
...
async def create_swap(
    ...
    try:
        ...
        # M7: requester_staff_id / target_staff_id / target_shift_id were
        # stored as-is from the request body with no org check. approve_swap
        # later does db.get(Shift, row.target_shift_id) and writes
        # tgt_shift.staff_id — a swap request created with a cross-org
        # target_shift_id/target_staff_id would let approval silently
        # reassign another organization's shift to this org's staff (or
        # vice versa). Validate all three here, at creation, the same way
        # requester_shift_id already is.
        requester_staff = await db.scalar(
            select(Staff).where(and_(Staff.id == body.requester_staff_id, Staff.org_id == member.org_id))
        )
        if not requester_staff:
            raise HTTPException(status_code=404, detail="Requester staff not found")
        target_staff = await db.scalar(
            select(Staff).where(and_(Staff.id == body.target_staff_id, Staff.org_id == member.org_id))
        )
        if not target_staff:
            raise HTTPException(status_code=404, detail="Target staff not found")
        if body.target_shift_id is not None:
            target_shift = await db.scalar(
                select(Shift).where(and_(Shift.id == body.target_shift_id, Shift.org_id == member.org_id))
            )
            if not target_shift:
