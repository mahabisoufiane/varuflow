...
router = APIRouter(prefix="/api/loyalty-referrals", tags=["loyalty-referrals"], dependencies=[Depends(require_module("invoicing"))])
...
async def qualify_referral(
    ...
    try:
        ...
        row.qualified_at = datetime.datetime.now(datetime.UTC)
        await db.commit()
        await db.refresh(row)
        return ReferralOut.model_validate(row)
    except HTTPException:
