...
# ── Brute-force lockout ───────────────────────────────────────────────────────
# After MAX_FAILED_ATTEMPTS within LOCKOUT_WINDOW seconds, the IP is locked
# out for LOCKOUT_DURATION seconds. This is independent of the rate-limit
# middleware and survives even if the attacker stays under the per-minute cap.
...
_lockouts: dict[str, float] = {}  # ip -> lockout_until (monotonic)
...
async def _record_failed(ip: str) -> None:
    """Record a failed attempt; engage lockout if threshold exceeded."""
    now = time.monotonic()
    ...
    async with _pin_lock:
        ...
        if len(attempts) >= _MAX_FAILED_ATTEMPTS:
            ...
            log.warning("POS PIN lockout engaged for ip=%s after %d failures", ip, len(attempts))


async def _clear_failed(ip: str) -> None:
...
@router.post(
    ...
    dependencies=[Depends(per_ip_rate_limit("pos.pin_login", 10, 900))],
)
async def authenticate_pos_device(
    ...
    for m in members:
        if m.pos_pin_hash and _bcrypt.checkpw(body.pin.encode(), m.pos_pin_hash.encode()):
            matched = m
            break

    if matched is None:
        # Constant-time dummy verify to prevent timing-based member enumeration
        _bcrypt.checkpw(body.pin.encode(), b"$2b$12$KIXAqrFMBMl3TQP8A0.qiu0Q8HwDQXq1JvxU1/VkU5X7RxFkm.Abe")
        await _record_failed(ip)
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid PIN")

    await _clear_failed(ip)
...
async def set_member_pin(
    ...
    target.pos_pin_hash = _bcrypt.hashpw(body.pin.encode(), _bcrypt.gensalt(12)).decode()
    await db.commit()
