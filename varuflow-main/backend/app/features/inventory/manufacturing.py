...
from .bom import BomHeader, BomLine
from app.services.audit import log_action
from .models import Product, StockLevel, StockMovement, StockMovementType, Warehouse
from .work_orders_mfg import WorkOrder, WorkOrderLabourLine, WorkOrderMaterialLine
...
async def _bom_would_create_cycle(
    org_id: uuid.UUID,
    bom_product_id: uuid.UUID,
    candidate_component_product_id: uuid.UUID,
    db: AsyncSession,
) -> bool:
    """Return True if adding candidate_component_product_id as a BOM line of
    bom_product_id would create a circular BOM dependency.

    Uses iterative BFS — walks the component tree of the candidate product
    to see if it eventually references bom_product_id.
    """
    if candidate_component_product_id == bom_product_id:
        return True  # direct self-reference

    visited: set[uuid.UUID] = set()
    frontier = [candidate_component_product_id]
    while frontier:
        current = frontier.pop()
        if current in visited:
            continue
        visited.add(current)
        # Find all active BOMs whose output product is `current`
        result = await db.execute(
            select(BomHeader.id).where(
                BomHeader.org_id == org_id,
                BomHeader.product_id == current,
                BomHeader.is_active.is_(True),
            )
        )
        bom_ids = [row[0] for row in result.all()]
        if not bom_ids:
            continue
        comp_result = await db.execute(
            select(BomLine.component_product_id).where(
                BomLine.bom_id.in_(bom_ids)
            )
        )
        for (comp_pid,) in comp_result.all():
            if comp_pid == bom_product_id:
...
async def add_bom_line(
    ...
    try:
        ...
        if not bom:
            raise HTTPException(status_code=404, detail="BOM not found")
        if await _bom_would_create_cycle(org_id, bom.product_id, body.component_product_id, db):
            raise HTTPException(
                status_code=422,
                detail="Circular BOM reference: the component's BOM tree already contains this BOM's product.",
            )
