"""Product notes router (Item 80).

Endpoints under ``/api/product-notes``:

    GET    ""                    list notes (filter by product_id)
    POST   ""                    create a note
    GET    /{note_id}             detail
...
author — backdating someone else's inventory context would silently
...
from .product_note import ProductNote
from app.services import product_note as svc_80
from app.services.audit import log_action
from app.middleware.plan_check import require_module

router = APIRouter(prefix="/api/product-notes", tags=["product-notes"], dependencies=[Depends(require_module("inventory"))])
...
async def _assert_product_belongs(
...
async def _load(
    ...
    row = await db.get(ProductNote, note_id)
    if row is None or row.org_id != org_id:
        raise HTTPException(status_code=404, detail="Note not found")
    return row


def _to_out(row: ProductNote) -> NoteOut:
    return NoteOut(
        id=row.id,
        ...
        mentions=svc_80.extract_mentions(row.body),
...
async def _count_pinned(
    ...
    stmt = select(func.count(ProductNote.id)).where(
        ProductNote.org_id == org_id,
        ProductNote.product_id == product_id,
        ProductNote.is_pinned.is_(True),
    )
    if exclude_id is not None:
        stmt = stmt.where(ProductNote.id != exclude_id)
...
async def list_notes(
    ...
    _user, member = ctx
    stmt = select(ProductNote).where(ProductNote.org_id == member.org_id)
    if product_id is not None:
        stmt = stmt.where(ProductNote.product_id == product_id)
    if pinned_only:
        stmt = stmt.where(ProductNote.is_pinned.is_(True))
    # Pinned bubble up; ties break by newest-first.
    stmt = stmt.order_by(
        ProductNote.is_pinned.desc(),
        ProductNote.created_at.desc(),
    ).limit(limit)
...
async def create_note(
    ...
    user, member = ctx
    await _assert_product_belongs(
        db, product_id=body.product_id, org_id=member.org_id,
    )
    try:
        text = svc_80.validate_body(body.body)
    ...
    if body.is_pinned:
        ...
        try:
            svc_80.assert_pin_limit(current_pinned=existing)
        except ValueError as e:
            raise HTTPException(status_code=400, detail=str(e))
    ...
    await log_action(
        ...
        target_type="product_note",
        target_id=str(row.id),
        request=request,
        extra={
            "product_id": str(body.product_id),
            "pinned":     bool(body.is_pinned),
            "mentions":   svc_80.extract_mentions(text),
        },
    )
    await db.commit()
...
async def update_note(
    ...
    await log_action(
        db,
        action="product_note.updated",
        org_id=member.org_id,
        actor_user_id=user["user_id"],
        target_type="product_note",
        target_id=str(row.id),
        request=request,
        extra={"mentions": svc_80.extract_mentions(text)},
    )
    await db.commit()
    await db.refresh(row)
    return _to_out(row)
