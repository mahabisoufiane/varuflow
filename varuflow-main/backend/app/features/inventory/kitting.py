...
from .kits import KitAssembly, KitComponent, KitDefinition


async def _kit_would_create_cycle(
    org_id: uuid.UUID,
    kit_product_id: uuid.UUID,
    candidate_component_product_ids: list[uuid.UUID],
    db: AsyncSession,
) -> bool:
    """Return True if adding any of the candidate products as components of
    kit_product_id would create a circular dependency.

    A cycle exists if the candidate product is itself a kit whose component
    tree (directly or transitively) contains kit_product_id.

    Uses an iterative BFS/DFS — avoids Python recursion depth limits for
    deeply nested kit trees, and avoids a raw recursive CTE so it works
    on async sessions without dropping into sync execution.
    """
    for candidate in candidate_component_product_ids:
        if candidate == kit_product_id:
            return True  # direct self-reference
        # BFS: expand the component tree of `candidate`
        ...
        while frontier:
            ...
            if current in visited:
                continue
            visited.add(current)
            # Find all kit definitions whose product_id == current
            result = await db.execute(
                select(KitDefinition.id).where(
                    KitDefinition.org_id == org_id,
                    KitDefinition.product_id == current,
                    KitDefinition.is_active.is_(True),
                )
            )
            kit_ids = [row[0] for row in result.all()]
            if not kit_ids:
                continue
            # Collect component_product_ids for all those kits
            comp_result = await db.execute(
                select(KitComponent.component_product_id).where(
                    KitComponent.kit_id.in_(kit_ids)
                )
            )
            for (comp_pid,) in comp_result.all():
                if comp_pid == kit_product_id:
                    return True  # cycle detected
                if comp_pid not in visited:
                    frontier.append(comp_pid)
    return False
...
async def create_kit(
    ...
    try:
        ...
        if await _kit_would_create_cycle(org_id, body.product_id, component_ids, db):
            raise HTTPException(
                status_code=422,
                detail="Circular kit reference: a component's kit tree already contains this kit's product.",
            )
        kit = KitDefinition(
            id=uuid.uuid4(),
            org_id=org_id,
...
async def replace_components(
    ...
    try:
        org_id = uuid.UUID(str(member["org_id"]))
        kit = await _get_kit(kit_id, org_id, db)
        candidate_ids = [c.component_product_id for c in components]
        if await _kit_would_create_cycle(org_id, kit.product_id, candidate_ids, db):
            raise HTTPException(
                status_code=422,
                detail="Circular kit reference: a component's kit tree already contains this kit's product.",
            )
