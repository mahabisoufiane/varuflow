...
from app.services import product_note as svc
...
MIGRATION_SRC = _read(
    "migrations/versions/b6c8d0e2f4a7_v84_product_notes.py"
)
MODEL_SRC   = _read("app/features/inventory/product_note.py")
SERVICE_SRC = _read("app/services/product_note.py")
ROUTER_SRC  = _read("app/features/inventory/product_notes.py")
...
def test_assert_pin_limit_under_cap_ok():
    ...
    svc.assert_pin_limit(current_pinned=svc.MAX_PINNED_PER_PRODUCT - 1)


def test_assert_pin_limit_at_cap_rejects():
    with pytest.raises(ValueError, match="pin limit"):
        svc.assert_pin_limit(current_pinned=svc.MAX_PINNED_PER_PRODUCT)


def test_assert_pin_limit_over_cap_rejects():
    with pytest.raises(ValueError):
        svc.assert_pin_limit(current_pinned=svc.MAX_PINNED_PER_PRODUCT + 5)
...
def test_migration_chain_from_v83():
    assert 'down_revision = "a5b7c9d1e3f6"' in MIGRATION_SRC
    assert 'revision = "b6c8d0e2f4a7"' in MIGRATION_SRC


def test_migration_creates_product_notes_table():
    assert '"product_notes"' in MIGRATION_SRC


def test_migration_cascades_on_org_and_product():
    # Deleting an org or a product must drop their notes.
    assert 'ForeignKey("organizations.id", ondelete="CASCADE")' in MIGRATION_SRC
    assert 'ForeignKey("products.id", ondelete="CASCADE")' in MIGRATION_SRC


def test_migration_has_hot_query_composite_index():
    assert "ix_product_notes_product_pin_created" in MIGRATION_SRC
    # Composite index in that exact order.
    assert '["product_id", "is_pinned", "created_at"]' in MIGRATION_SRC
...
def test_model_declares_all_fields():
    for f in (
        "org_id", "product_id", "author_user_id", "body",
...
def test_router_prefix_and_endpoints():
    assert 'prefix="/api/product-notes"' in ROUTER_SRC
    for path in (
        '@router.get("", ',
        '@router.post(\n    "",',
        '@router.get("/{note_id}"',
...
def test_router_list_pins_bubble_to_top():
    assert "ProductNote.is_pinned.desc()" in ROUTER_SRC
    assert "ProductNote.created_at.desc()" in ROUTER_SRC


def test_router_tenant_scopes_every_query():
    # SQL-level org scope on list and helper count queries.
    assert "ProductNote.org_id == member.org_id" in ROUTER_SRC
    assert "ProductNote.org_id == org_id" in ROUTER_SRC
    # Row-level guard inside _load() for by-id fetches.
    assert "row.org_id != org_id" in ROUTER_SRC


def test_router_product_belongs_check():
    assert "_assert_product_belongs" in ROUTER_SRC
...
def test_router_registered_in_main():
    ...
    assert "product_notes" in feat_src
    assert "inventory_router" in MAIN_SRC


def test_router_imports_product_from_inventory():
    # Product model lives in inventory.py.
    assert "from app.features.inventory.models import Product" in ROUTER_SRC
