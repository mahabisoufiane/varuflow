...
from app.services import invoice_tag as svc
...
ROUTER_SRC    = _read("app/routers/invoice_tags.py")
MIGRATION_SRC = _read(
    "migrations/versions/a2b4c6d8e0f5_v89_invoice_tags.py"
)
MODEL_SRC     = _read("app/models/invoice_tag.py")
...
def test_constants_are_sane():
    assert svc.MAX_TAGS_PER_INVOICE == 20
...
def test_migration_chains_from_v88():
    assert 'down_revision = "f0a2b4c6d8e3"' in MIGRATION_SRC
    assert 'revision = "a2b4c6d8e0f5"' in MIGRATION_SRC


def test_migration_creates_both_tables():
    assert '"invoice_tags"' in MIGRATION_SRC
    assert '"invoice_tag_assignments"' in MIGRATION_SRC
...
def test_migration_assignment_has_composite_pk():
    ...
    assert '"invoice_id"' in MIGRATION_SRC
...
def test_model_tablenames():
    assert '__tablename__ = "invoice_tags"' in MODEL_SRC
    assert '__tablename__ = "invoice_tag_assignments"' in MODEL_SRC
...
def test_router_prefix():
    assert 'prefix="/api/invoice-tags"' in ROUTER_SRC


def test_router_has_all_endpoints():
    # 5 on the tag itself + 3 on the relationship + 1 for inv→tags.
    ...
    assert '@router.get("/{tag_id}/invoices"' in ROUTER_SRC
    assert '@router.post("/assignments"' in ROUTER_SRC
    assert '@router.delete("/assignments"' in ROUTER_SRC
    assert '@router.get("/invoices/{invoice_id}"' in ROUTER_SRC


def test_router_uses_pure_service():
    assert "svc_87.normalize_name" in ROUTER_SRC
    assert "svc_87.normalize_color" in ROUTER_SRC
    assert "svc_87.assert_under_limit" in ROUTER_SRC


def test_router_tenant_scope_on_every_load():
    # _load_tag and _load_invoice both compare .org_id → org_id
    assert "row.org_id != org_id" in ROUTER_SRC
    # The list and the count queries filter on the scoped org too.
    assert "InvoiceTag.org_id == member.org_id" in ROUTER_SRC
    assert "Invoice.org_id == member.org_id" in ROUTER_SRC
...
def test_router_assignment_is_idempotent():
    ...
    # must not raise on a second call with the same (invoice, tag).
