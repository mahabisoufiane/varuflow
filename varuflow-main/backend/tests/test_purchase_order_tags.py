...
from app.services import purchase_order_tag as svc
...
SERVICE_SRC   = _read("app/services/purchase_order_tag.py")
ROUTER_SRC    = _read("app/routers/purchase_order_tags.py")
MIGRATION_SRC = _read(
    "migrations/versions/c6d8e0f2a4b9_v91_purchase_order_tags.py"
)
MODEL_SRC     = _read("app/models/purchase_order_tag.py")
...
def test_name_trim_and_collapse_whitespace():
    assert svc.normalize_name("  Rush   shipping ") == "Rush shipping"


def test_name_rejects_non_string():
    for bad in (None, 42, b"rush", ["rush"]):
...
def test_name_rejects_control_chars():
    with pytest.raises(ValueError):
        svc.normalize_name("rush\x00bad")
...
def test_name_preserves_non_ascii():
    assert svc.normalize_name("Årsfaktura") == "Årsfaktura"
...
def test_keys_equal_case_insensitive():
    assert svc.keys_equal("Rush", "rush")
    assert svc.keys_equal("Rush  order", " rush   order ")


def test_keys_equal_different_names_not_equal():
    assert not svc.keys_equal("Rush", "Disputed")
...
def test_migration_chains_from_v90():
    assert 'down_revision = "b4c6d8e0f2a7"' in MIGRATION_SRC
    assert 'revision = "c6d8e0f2a4b9"' in MIGRATION_SRC


def test_migration_creates_both_tables():
    assert '"purchase_order_tags"' in MIGRATION_SRC
    assert '"purchase_order_tag_assignments"' in MIGRATION_SRC


def test_migration_case_insensitive_unique_index():
    # Must use a functional expression on lower(name) so "Rush" and
    # "rush" collide at the database level.
    assert "ux_purchase_order_tags_org_name_lower" in MIGRATION_SRC
    assert "lower(name)" in MIGRATION_SRC
    assert "unique=True" in MIGRATION_SRC


def test_migration_cascades_on_org_and_po():
    # Each FK to organizations / purchase_orders / tags should cascade
    # so deleting a parent row cleans up every dependent assignment.
...
def test_migration_assignment_has_composite_pk():
    ...
    assert '"purchase_order_id"' in MIGRATION_SRC
...
def test_model_tablenames():
    assert '__tablename__ = "purchase_order_tags"' in MODEL_SRC
    assert '__tablename__ = "purchase_order_tag_assignments"' in MODEL_SRC
...
def test_router_prefix():
    assert 'prefix="/api/purchase-order-tags"' in ROUTER_SRC


def test_router_has_all_endpoints():
    # 5 on the tag itself + 3 on the relationship + 1 for po→tags.
    ...
    assert '"/{tag_id}/purchase-orders"' in ROUTER_SRC
    assert '@router.post("/assignments"' in ROUTER_SRC
    assert '@router.delete("/assignments"' in ROUTER_SRC
    assert '"/purchase-orders/{purchase_order_id}"' in ROUTER_SRC


def test_router_uses_pure_service():
    assert "svc_90.normalize_name" in ROUTER_SRC
    assert "svc_90.normalize_color" in ROUTER_SRC
    assert "svc_90.assert_under_limit" in ROUTER_SRC


def test_router_tenant_scope_on_every_load():
    # _load_tag and _load_po both compare .org_id → org_id
    assert "row.org_id != org_id" in ROUTER_SRC
    # The list and the count queries filter on the scoped org too.
    assert "PurchaseOrderTag.org_id == member.org_id" in ROUTER_SRC
    assert "PurchaseOrder.org_id == member.org_id" in ROUTER_SRC


def test_router_emits_five_audit_actions():
    for action in (
        "purchase_order_tag.created",
        "purchase_order_tag.updated",
        "purchase_order_tag.deleted",
...
def test_router_assignment_is_idempotent():
    ...
    # must not raise on a second call with the same (po, tag).
...
def test_router_imports_purchase_order_from_inventory():
    assert "from app.models.inventory import PurchaseOrder" in ROUTER_SRC


def test_router_registered_in_main():
    assert "purchase_order_tags.router" in MAIN_SRC
    assert "purchase_order_tags," in MAIN_SRC
