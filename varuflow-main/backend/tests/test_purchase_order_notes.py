...
from app.services import purchase_order_note as svc
...
MIGRATION_SRC = _read(
    "migrations/versions/b4c6d8e0f2a7_v90_purchase_order_notes.py"
)
MODEL_SRC   = _read("app/models/purchase_order_note.py")
SERVICE_SRC = _read("app/services/purchase_order_note.py")
ROUTER_SRC  = _read("app/routers/purchase_order_notes.py")
...
def test_assert_pin_limit_under_cap_ok():
    ...
    svc.assert_pin_limit(current_pinned=svc.MAX_PINNED_PER_PO - 1)


def test_assert_pin_limit_at_cap_rejects():
    with pytest.raises(ValueError, match="pin limit"):
        svc.assert_pin_limit(current_pinned=svc.MAX_PINNED_PER_PO)


def test_assert_pin_limit_over_cap_rejects():
    with pytest.raises(ValueError):
        svc.assert_pin_limit(current_pinned=svc.MAX_PINNED_PER_PO + 5)
...
def test_migration_chain_from_v89():
    assert 'down_revision = "a2b4c6d8e0f5"' in MIGRATION_SRC
    assert 'revision = "b4c6d8e0f2a7"' in MIGRATION_SRC


def test_migration_creates_purchase_order_notes_table():
    assert '"purchase_order_notes"' in MIGRATION_SRC


def test_migration_cascades_on_org_and_po():
    # Deleting an org or a PO must drop their notes.
    assert 'ForeignKey("organizations.id", ondelete="CASCADE")' in MIGRATION_SRC
    assert 'ForeignKey("purchase_orders.id", ondelete="CASCADE")' in MIGRATION_SRC


def test_migration_has_hot_query_composite_index():
    assert "ix_purchase_order_notes_po_pin_created" in MIGRATION_SRC
    # Composite index in that exact order.
    assert '["purchase_order_id", "is_pinned", "created_at"]' in MIGRATION_SRC
...
def test_model_declares_all_fields():
    for f in (
        "org_id", "purchase_order_id", "author_user_id", "body",
...
def test_model_tablename():
    assert '__tablename__ = "purchase_order_notes"' in MODEL_SRC
...
def test_router_prefix_and_endpoints():
    assert 'prefix="/api/purchase-order-notes"' in ROUTER_SRC
    for path in (
        '@router.get("", ',
        '@router.post(\n    "",',
        '@router.get("/{note_id}"',
...
def test_router_list_pins_bubble_to_top():
    assert "PurchaseOrderNote.is_pinned.desc()" in ROUTER_SRC
    assert "PurchaseOrderNote.created_at.desc()" in ROUTER_SRC


def test_router_tenant_scopes_every_query():
    assert "PurchaseOrderNote.org_id == member.org_id" in ROUTER_SRC
    assert "PurchaseOrderNote.org_id == org_id" in ROUTER_SRC
    # Row-level guard inside _load() for by-id fetches.
    assert "row.org_id != org_id" in ROUTER_SRC


def test_router_po_belongs_check():
    assert "_assert_po_belongs" in ROUTER_SRC
    assert "PurchaseOrder.org_id == org_id" in ROUTER_SRC
...
def test_router_registered_in_main():
    assert "purchase_order_notes.router" in MAIN_SRC
    assert "purchase_order_notes," in MAIN_SRC


def test_router_imports_purchase_order_from_inventory():
    # PurchaseOrder model lives in inventory.py.
