...
function Avatar({ name }: { name: string }) {
  ...
  return (
    <div className={cn("flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br text-[13px] font-bold text-white", color)}>
      {initials}
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="vf-input pl-9 pr-9"
        />
        {search && (
          <button onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2">
            <X className="h-4 w-4 text-slate-600 hover:text-slate-300 transition-colors" />
          </button>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-white/[0.08] bg-vf-surface px-6 py-20 text-center">
          ...
          {!search && (
            <button onClick={openCreate} className="mt-4 vf-btn text-xs px-4 py-2 inline-flex">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {filtered.map(c => (
            <div key={c.id}
              className="group flex items-start gap-4 rounded-xl border border-white/[0.06] bg-vf-surface p-5 hover:border-white/[0.10] hover:bg-vf-elevated transition-all">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 space-y-1">
                  {c.email && (
                    <div className="flex items-center gap-1.5 text-xs text-slate-500">
                      <Mail className="h-3 w-3 shrink-0 text-slate-600" />{c.email}
                    </div>
                  )}
                  {c.phone && (
                    <div className="flex items-center gap-1.5 text-xs text-slate-500">
                      <Phone className="h-3 w-3 shrink-0 text-slate-600" />{c.phone}
                    </div>
                  )}
                  {c.org_number && (
                    <div className="flex items-center gap-1.5 text-xs text-slate-500">
                      <Building2 className="h-3 w-3 shrink-0 text-slate-600" />Org {c.org_number}
                    </div>
                  )}
                </div>
                ...
                <div className="mt-3 flex items-center gap-2">
                  <button onClick={() => openEdit(c)}
                    className="rounded-lg border border-white/[0.08] px-2.5 py-1 text-[11px] font-semibold text-slate-500 hover:bg-white/5 hover:text-slate-300 transition-colors">
                    Edit
                  </button>
                  <a href={`/invoices/new?customer=${c.id}`}
                    className="inline-flex items-center gap-1 rounded-lg border border-indigo-500/20 bg-indigo-500/10 px-2.5 py-1 text-[11px] font-semibold text-indigo-400 hover:bg-indigo-500/20 transition-colors">
                    <FileText className="h-3 w-3" />Invoice
                  </a>
                  <a href={`/invoices?customer=${c.id}`}
                    className="ml-auto inline-flex items-center gap-1 text-[11px] text-slate-600 hover:text-slate-300 transition-colors">
          ...
          <form onSubmit={handleSave} className="space-y-4 pt-1">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={id} className={cn("space-y-1", span ? "col-span-2" : "")}>
              ...
              <div className="space-y-1">
                <Label htmlFor="payment_terms_days" className="text-xs text-slate-400">Payment terms (days)</Label>
                <input
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button type="button" onClick={() => setOpen(false)} className="vf-btn-ghost text-xs px-4 py-2">
                Cancel
              </button>
              <button type="submit" disabled={saving} className="vf-btn text-xs px-4 py-2 disabled:opacity-50">
