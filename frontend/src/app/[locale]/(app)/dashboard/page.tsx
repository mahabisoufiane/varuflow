...
/* ── Sparkline — unique gradient ID via useId ────────────────────────────────── */
function Sparkline({ data, color = "#3b82f6", w = 72, h = 30 }: {
...
/* ── MiniBar chart (inline SVG, no Recharts) ─────────────────────────────────── */
      ...
      {data.map((v, i) => {
        ...
        return (
          <div
            key={i}
            style={{ height: `${pct}%` }}
            className={`flex-1 rounded-[3px] transition-all ${isLast ? "bg-blue-400" : "bg-white/[0.12]"}`}
          />
        );
      })}
    </div>
...
function StatusPill({ overdue }: { overdue: boolean }) {
  if (!overdue) return <span className="h-1.5 w-1.5 rounded-full bg-blue-400 shrink-0" />;
  return (
    <span className="shrink-0 rounded-full bg-red-50 border border-red-200 px-1.5 py-0.5 text-[10px] font-semibold text-red-600 leading-none">
      Overdue
    </span>
  );
}
...
export default function DashboardPage() {
  ...
  // All Date logic is client-only to avoid hydration mismatches
  ...
  useEffect(() => {
    Promise.all([
      ...
      setOverview(ov);
      // Format movement dates client-side to avoid SSR mismatch
  ...
  if (loading) return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="space-y-1.5"><Sk className="h-3.5 w-32" /><Sk className="h-7 w-24" /></div>
        <div className="flex gap-2"><Sk className="h-8 w-24" /><Sk className="h-8 w-24" /></div>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[0,1,2,3].map(i => <Sk key={i} className="h-28" />)}
      </div>
      <div className="grid md:grid-cols-3 gap-4">
        <Sk className="md:col-span-2 h-52" />
        <Sk className="h-52" />
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs text-gray-400 capitalize tracking-wide" suppressHydrationWarning>{todayLabel}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* Dark hero */}
        <div className="md:col-span-2 relative overflow-hidden rounded-2xl bg-[#0c111c] p-6 text-white shadow-xl">
          {/* Glows */}
          <div className="pointer-events-none absolute inset-0 overflow-hidden">
            <div className="absolute -top-12 -right-12 h-56 w-56 rounded-full bg-blue-500/10 blur-3xl" />
            <div className="absolute bottom-0 left-4 h-40 w-40 rounded-full bg-indigo-600/8 blur-2xl" />
          </div>
          ...
          <div className="relative">
            ...
            <div className="flex items-start justify-between mb-5">
              <div>
                <p className="text-[10px] text-blue-200/50 font-semibold uppercase tracking-[0.14em]">Total receivables</p>
                <p className="mt-1.5 text-[38px] font-bold tracking-tight leading-none">
                  {fmt(outstanding)}
                  <span className="ml-1.5 text-xl font-normal text-blue-200/35">SEK</span>
                </p>
            ...
            {revData.length > 1 && (
              <div className="mb-5">
                <p className="text-[10px] text-blue-200/35 uppercase tracking-[0.12em] mb-2.5">
            ...
            <div className="flex items-center gap-2">
              <Link href="/invoices?status=SENT"
                className="inline-flex items-center gap-1.5 rounded-lg bg-white/10 px-3.5 py-2 text-xs font-semibold text-white hover:bg-white/15 transition-colors">
                View invoices <ArrowUpRight className="h-3 w-3" />
              </Link>
              <Link href="/analytics"
                className="inline-flex items-center gap-1.5 rounded-lg bg-white/5 px-3.5 py-2 text-xs font-medium text-blue-200/60 hover:bg-white/10 hover:text-white transition-colors">
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-4 flex flex-col">
          ...
          <div className="flex flex-col gap-0.5 flex-1">
            {[
              { href: "/invoices/new",            icon: FileText,    label: "New invoice",    col: "text-blue-600 bg-blue-50" },
              { href: "/inventory/products/new",  icon: Package,     label: "Add product",    col: "text-amber-600 bg-amber-50" },
              { href: "/customers/new",           icon: Users,       label: "Add customer",   col: "text-emerald-600 bg-emerald-50" },
              { href: "/pos",                     icon: ShoppingCart,label: "Open register",  col: "text-violet-600 bg-violet-50" },
              { href: "/ai",                      icon: Zap,         label: "AI insights",    col: "text-indigo-600 bg-indigo-50" },
            ].map(({ href, icon: Icon, label, col }) => (
              <Link key={href} href={href}
                className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors group">
                <span className={`inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${col}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100">
            <h2 className="flex items-center gap-2 text-sm font-semibold text-gray-900">
              <FileText className="h-4 w-4 text-blue-500" />Awaiting payment
            </h2>
          ...
          {openInvoices.length === 0 ? (
            <div className="py-10 text-center">
              <CheckCircle2 className="mx-auto h-8 w-8 text-emerald-200 mb-2" />
            ...
            <div className="divide-y divide-gray-50">
              {openInvoices.map((inv) => {
                ...
                return (
                  <div key={inv.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50/70 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 truncate">{inv.customer.company_name}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="font-mono text-sm font-semibold text-gray-900">{fmt(Number(inv.total_sek))} kr</p>
                      <p className="text-[11px] text-gray-400">Due {inv.due_date}</p>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100">
            ...
            <Link href="/inventory" className="flex items-center gap-1 text-xs text-gray-400 hover:text-gray-700 transition-colors">
              All <ArrowRight className="h-3 w-3" />
            </Link>
          </div>
          {lowStock.length === 0 ? (
                ...
                return (
                  <div key={i} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50/70 transition-colors">
                    <div className={`shrink-0 flex h-7 w-7 items-center justify-center rounded-lg
                      ...
                      <AlertTriangle className={`h-3.5 w-3.5 ${urgent ? "text-red-500" : "text-amber-500"}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-900 truncate">{sl.product.name}</p>
                      <p className="text-xs text-gray-400">{sl.product.sku} · {sl.warehouse.name}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className={`text-sm font-bold tabular-nums ${urgent ? "text-red-600" : "text-amber-600"}`}>
                        {sl.quantity}
                      </p>
                      <p className="text-[11px] text-gray-400">min {sl.min_threshold}</p>
                    </div>
                  </div>
                );
              })}
      ...
      {recentMovements.length > 0 && (
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100">
            <h2 className="flex items-center gap-2 text-sm font-semibold text-gray-900">
              <Activity className="h-4 w-4 text-gray-400" />Recent movements
            </h2>
          ...
          <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
            {recentMovements.map((m) => (
              <div key={m.id} className="flex items-center gap-2.5 rounded-lg bg-gray-50/80 px-3 py-2.5">
                <span className={`shrink-0 w-8 rounded-md py-0.5 text-center text-[10px] font-bold ${
                  m.type === "IN"  ? "bg-emerald-100 text-emerald-700" :
                  m.type === "OUT" ? "bg-red-100 text-red-700" : "bg-gray-200 text-gray-600"
                }`}>{m.type}</span>
                <span className="flex-1 min-w-0 truncate text-sm font-medium text-gray-800">{m.product.name}</span>
                <span className="shrink-0 tabular-nums text-sm font-semibold text-gray-600">{m.quantity}</span>
                <span className="shrink-0 text-[11px] text-gray-400">{movementDates[m.id] ?? ""}</span>
              </div>
            ))}
          </div>
        </div>
...
function KpiCard({ label, value, sub, trend, icon, iconCls, href, spark, sparkColor }: {
  ...
  return (
    <Link href={href}
      className="group relative overflow-hidden rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:border-gray-300 hover:shadow-md transition-all">
      <div className="flex items-start justify-between mb-3">
        <span className={`inline-flex h-8 w-8 items-center justify-center rounded-lg ${iconCls}`}>{icon}</span>
      ...
      {trend && (
        <p className={`mt-2 inline-flex items-center gap-0.5 text-[11px] font-medium
