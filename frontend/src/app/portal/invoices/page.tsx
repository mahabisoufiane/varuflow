...
export default function PortalInvoicesPage() {
  ...
  useEffect(() => {
    ...
    if (info) {
      // Guard against a malformed / user-edited storage entry — otherwise
      // JSON.parse throws and the effect aborts before the invoice fetch.
      try {
        const parsed = JSON.parse(info);
        if (parsed && typeof parsed === "object") {
          setCustomerName(typeof parsed.customer_name === "string" ? parsed.customer_name : "");
          setOrgName(typeof parsed.org_name === "string" ? parsed.org_name : "");
        }
      } catch {
          ...
          <button
            onClick={async () => {
              try {
                ...
                const res = await fetch(`${base}/api/portal/invoices-zip`, {
                  headers: token ? { Authorization: `Bearer ${token}` } : {},
                });
                if (!res.ok) throw new Error("Download failed");
                const blob = await res.blob();
                const url = URL.createObjectURL(blob);
                Object.assign(document.createElement("a"), { href: url, download: "invoices.zip" }).click();
                URL.revokeObjectURL(url);
              } catch {
                // toast would require import; use simple alert
