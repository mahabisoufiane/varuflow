...
- CountryMiddleware and RequestIdMiddleware are registered before CORSMiddleware so they are inner layers and never produce responses without CORS headers
...
⚠️ **94 migration files exist** — the table below lists only the foundational ones for orientation.
...
| lib/portal-client.ts | HTTP client for portal routes (attaches portal JWT, not Supabase token). 8s timeout, 401 → clears session, redirects to /portal/login |
| lib/supplier-portal-client.ts | HTTP client for supplier portal routes (attaches supplier JWT, distinct localStorage key). 8s timeout, 401 → clears session, redirects to /supplier-portal |
...
| File | Responsibility |
|------|---------------|
| i18n/routing.ts | Active locales: sv, en. Default: sv. (no/da message files exist but are NOT in routing — add them here to activate) |
...
| frontend/middleware.ts | Next.js entry point — re-exports default + config from proxy.ts |
| frontend/src/proxy.ts | Full middleware: excludes /portal/*, refreshes Supabase session, auth redirects, next-intl locale routing |
...
| lib/api-client.ts | Authenticated HTTP client. Auto-refreshes token within 60s of expiry. 10s AbortController timeout |
