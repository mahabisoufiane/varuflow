"""Pure helpers for supplier notes (Item 76).
...
* Pin-limit guard — a single supplier may not hold more than
  ``MAX_PINNED_PER_SUPPLIER`` pinned notes at once, to keep the
...
def assert_pin_limit(*, current_pinned: int) -> None:
    """Reject a pin that would push the count over the per-supplier cap.

    ``current_pinned`` is the count *excluding* the row being pinned
    — so an unpinned note can always be pinned if
    ``current_pinned < MAX_PINNED_PER_SUPPLIER``.
    """
    if current_pinned < 0:
        raise ValueError("current_pinned cannot be negative")
    if current_pinned >= MAX_PINNED_PER_SUPPLIER:
        raise ValueError(
            f"pin limit reached ({MAX_PINNED_PER_SUPPLIER} notes per supplier)"
        )
