"""Pure helpers for customer contacts (Item 74).
...
MAX_CONTACTS_PER_CUSTOMER: int = 50
...
def assert_under_limit(*, current_count: int) -> None:
    ...
    if current_count >= MAX_CONTACTS_PER_CUSTOMER:
        raise ValueError(
            f"customer already has {MAX_CONTACTS_PER_CUSTOMER} contacts "
            "(limit reached)"
        )
