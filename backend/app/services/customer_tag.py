"""Pure helpers for customer tags (Item 73).
...
def normalize_name(raw: object) -> str:
    ...
    # Collapse internal whitespace to single spaces so "  VIP   client "
    # and "VIP client" produce the same canonical name.
...
def assert_under_limit(*, current_count: int) -> None:
    ...
    if current_count >= MAX_TAGS_PER_CUSTOMER:
        raise ValueError(
            f"customer already has {MAX_TAGS_PER_CUSTOMER} tags "
            "(limit reached)"
        )
