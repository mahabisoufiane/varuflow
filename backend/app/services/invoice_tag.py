"""Pure helpers for invoice tags (Item 87).
...
def normalize_name(raw: object) -> str:
    ...
    # Collapse internal whitespace to single spaces so "  Rush  " and
    # "Rush" produce the same canonical name.
...
def assert_under_limit(*, current_count: int) -> None:
    ...
    if current_count >= MAX_TAGS_PER_INVOICE:
        raise ValueError(
            f"invoice already has {MAX_TAGS_PER_INVOICE} tags "
            "(limit reached)"
        )
