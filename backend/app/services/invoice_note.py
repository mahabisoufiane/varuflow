"""Pure helpers for invoice notes (Item 86).
...
* Pin-limit guard — a single invoice may not hold more than
  ``MAX_PINNED_PER_INVOICE`` pinned notes at once, to keep the
...
def assert_pin_limit(*, current_pinned: int) -> None:
    """Reject a pin that would push the count over the per-invoice cap.

    ``current_pinned`` is the count *excluding* the row being pinned
    — so an unpinned note can always be pinned if
    ``current_pinned < MAX_PINNED_PER_INVOICE``.
    """
    if current_pinned < 0:
        raise ValueError("current_pinned cannot be negative")
    if current_pinned >= MAX_PINNED_PER_INVOICE:
        raise ValueError(
            f"pin limit reached ({MAX_PINNED_PER_INVOICE} notes per invoice)"
        )
