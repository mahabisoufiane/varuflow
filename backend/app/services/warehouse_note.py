"""Pure helpers for warehouse notes (Item 83).
...
* Pin-limit guard — a single warehouse may not hold more than
  ``MAX_PINNED_PER_WAREHOUSE`` pinned notes at once, to keep the
  "pinned" area scannable.
"""
...
MAX_PINNED_PER_WAREHOUSE: int = 5
...
def assert_pin_limit(*, current_pinned: int) -> None:
    """Reject a pin that would push the count over the per-warehouse cap.

    ``current_pinned`` is the count *excluding* the row being pinned
    — so an unpinned note can always be pinned if
    ``current_pinned < MAX_PINNED_PER_WAREHOUSE``.
    """
    if current_pinned < 0:
        raise ValueError("current_pinned cannot be negative")
    if current_pinned >= MAX_PINNED_PER_WAREHOUSE:
        raise ValueError(
            f"pin limit reached ({MAX_PINNED_PER_WAREHOUSE} notes per warehouse)"
        )
