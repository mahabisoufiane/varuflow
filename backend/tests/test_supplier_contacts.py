...
from app.services import supplier_contact as svc
...
SERVICE_SRC   = _read("app/services/supplier_contact.py")
ROUTER_SRC    = _read("app/routers/supplier_contacts.py")
MIGRATION_SRC = _read("migrations/versions/a5b7c9d1e3f6_v83_supplier_contacts.py")
MODEL_SRC     = _read("app/models/supplier_contact.py")
...
def test_role_trims_and_keeps_value():
    assert svc.normalize_role("  Sales  ") == "Sales"
...
def test_constants_are_sane():
    assert svc.MAX_CONTACTS_PER_SUPPLIER == 50
...
def test_migration_chains_from_v82():
    assert 'down_revision = "f4a6b8d0c2e5"' in MIGRATION_SRC
    assert 'revision = "a5b7c9d1e3f6"' in MIGRATION_SRC


def test_migration_creates_table():
    assert '"supplier_contacts"' in MIGRATION_SRC
    for col in (
...
def test_migration_partial_unique_primary_index():
    # A supplier can have at most one primary contact; multiple
    # non-primary rows are OK, so the uniqueness is partial.
    assert "ux_supplier_contacts_one_primary_per_supplier" in MIGRATION_SRC
...
def test_model_uses_encrypted_string_for_pii():
    # email + phone are PII — same treatment as suppliers.email / phone.
...
def test_model_tablename_and_columns():
    assert '__tablename__ = "supplier_contacts"' in MODEL_SRC
    for col in (
        "is_primary", "receives_rfq", "role", "supplier_id",
        "org_id", "updated_at",
    ):
...
def test_router_prefix():
    assert 'prefix="/api/supplier-contacts"' in ROUTER_SRC
...
def test_router_uses_pure_service():
    for fn in (
        ...
        assert f"svc_78.{fn}" in ROUTER_SRC


def test_router_tenant_scope_at_row_and_query_level():
    assert "row.org_id != org_id" in ROUTER_SRC
    assert "SupplierContact.org_id == member.org_id" in ROUTER_SRC
...
def test_router_invariant_channel_on_update():
    ...
    assert "svc_78.assert_has_channel(email=row.email" in ROUTER_SRC
...
def test_router_registered_in_main():
    assert "supplier_contacts.router" in MAIN_SRC
    assert "supplier_contacts," in MAIN_SRC
