...
from app.services import supplier_note as svc
...
MIGRATION_SRC = _read(
    "migrations/versions/e3f5a7b9c0d4_v81_supplier_notes.py"
)
MODEL_SRC   = _read("app/models/supplier_note.py")
SERVICE_SRC = _read("app/services/supplier_note.py")
ROUTER_SRC  = _read("app/routers/supplier_notes.py")
...
def test_assert_pin_limit_under_cap_ok():
    ...
    svc.assert_pin_limit(current_pinned=svc.MAX_PINNED_PER_SUPPLIER - 1)


def test_assert_pin_limit_at_cap_rejects():
    with pytest.raises(ValueError, match="pin limit"):
        svc.assert_pin_limit(current_pinned=svc.MAX_PINNED_PER_SUPPLIER)


def test_assert_pin_limit_over_cap_rejects():
    with pytest.raises(ValueError):
        svc.assert_pin_limit(current_pinned=svc.MAX_PINNED_PER_SUPPLIER + 5)
...
def test_migration_chain_from_v80():
    assert 'down_revision = "d2e4f6a8b0c3"' in MIGRATION_SRC
    assert 'revision = "e3f5a7b9c0d4"' in MIGRATION_SRC


def test_migration_creates_supplier_notes_table():
    assert '"supplier_notes"' in MIGRATION_SRC


def test_migration_cascades_on_org_and_supplier():
    # Deleting an org or a supplier must drop their notes.
    assert 'ForeignKey("organizations.id", ondelete="CASCADE")' in MIGRATION_SRC
    assert 'ForeignKey("suppliers.id", ondelete="CASCADE")' in MIGRATION_SRC


def test_migration_has_hot_query_composite_index():
    assert "ix_supplier_notes_supplier_pin_created" in MIGRATION_SRC
    # Composite index in that exact order.
    assert '["supplier_id", "is_pinned", "created_at"]' in MIGRATION_SRC
...
def test_model_declares_all_fields():
    for f in (
        "org_id", "supplier_id", "author_user_id", "body",
...
def test_router_prefix_and_endpoints():
    assert 'prefix="/api/supplier-notes"' in ROUTER_SRC
    for path in (
        '@router.get("", ',
        '@router.post(\n    "",',
        '@router.get("/{note_id}"',
...
def test_router_list_pins_bubble_to_top():
    assert "SupplierNote.is_pinned.desc()" in ROUTER_SRC
    assert "SupplierNote.created_at.desc()" in ROUTER_SRC


def test_router_tenant_scopes_every_query():
    # SQL-level org scope on list and helper count queries.
    assert "SupplierNote.org_id == member.org_id" in ROUTER_SRC
    assert "SupplierNote.org_id == org_id" in ROUTER_SRC
    # Row-level guard inside _load() for by-id fetches.
    assert "row.org_id != org_id" in ROUTER_SRC


def test_router_supplier_belongs_check():
...
def test_router_registered_in_main():
    assert "supplier_notes.router" in MAIN_SRC
    assert "supplier_notes," in MAIN_SRC
