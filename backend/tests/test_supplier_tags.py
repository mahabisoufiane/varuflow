...
from app.services import supplier_tag as svc
...
def test_name_trim_and_collapse_whitespace():
    assert svc.normalize_name("  Preferred   vendor ") == "Preferred vendor"


def test_name_rejects_non_string():
    for bad in (None, 42, b"preferred", ["preferred"]):
...
def test_name_rejects_control_chars():
    with pytest.raises(ValueError):
        svc.normalize_name("pref\x00bad")
...
def test_name_preserves_non_ascii():
    assert svc.normalize_name("Överleverantör") == "Överleverantör"
...
def test_keys_equal_case_insensitive():
    assert svc.keys_equal("Preferred", "preferred")
    assert svc.keys_equal("Preferred vendor", " preferred   vendor ")


def test_keys_equal_different_names_not_equal():
    assert not svc.keys_equal("Preferred", "Backup")
...
def test_constants_are_sane():
    assert svc.MAX_TAGS_PER_SUPPLIER == 20
...
def test_migration_chains_from_v81():
    assert 'down_revision = "e3f5a7b9c0d4"' in MIGRATION_SRC
    assert 'revision = "f4a6b8d0c2e5"' in MIGRATION_SRC


def test_migration_creates_both_tables():
    assert '"supplier_tags"' in MIGRATION_SRC
    assert '"supplier_tag_assignments"' in MIGRATION_SRC


def test_migration_case_insensitive_unique_index():
    # Must use a functional expression on lower(name) so "Preferred"
    # and "preferred" collide at the database level.
    assert "ux_supplier_tags_org_name_lower" in MIGRATION_SRC
    assert "lower(name)" in MIGRATION_SRC
    assert "unique=True" in MIGRATION_SRC


def test_migration_cascades_on_org_and_supplier():
    # Each FK to organizations / suppliers / tags should cascade so
...
def test_migration_assignment_has_composite_pk():
    ...
    assert '"supplier_id"' in MIGRATION_SRC
...
def test_model_tablenames():
    assert '__tablename__ = "supplier_tags"' in MODEL_SRC
    assert '__tablename__ = "supplier_tag_assignments"' in MODEL_SRC
...
def test_router_prefix():
    assert 'prefix="/api/supplier-tags"' in ROUTER_SRC


def test_router_has_all_endpoints():
    # 5 on the tag itself + 3 on the relationship + 1 for supplier→tags.
    ...
    assert '@router.get("/{tag_id}/suppliers"' in ROUTER_SRC
    assert '@router.post("/assignments"' in ROUTER_SRC
    assert '@router.delete("/assignments"' in ROUTER_SRC
    assert '@router.get("/suppliers/{supplier_id}"' in ROUTER_SRC


def test_router_uses_pure_service():
    assert "svc_77.normalize_name" in ROUTER_SRC
    assert "svc_77.normalize_color" in ROUTER_SRC
    assert "svc_77.assert_under_limit" in ROUTER_SRC


def test_router_tenant_scope_on_every_load():
    # _load_tag and _load_supplier both compare .org_id → org_id
    assert "row.org_id != org_id" in ROUTER_SRC
    # The list and the count queries filter on the scoped org too.
    assert "SupplierTag.org_id == member.org_id" in ROUTER_SRC
    assert "Supplier.org_id == member.org_id" in ROUTER_SRC


def test_router_emits_five_audit_actions():
    for action in (
        "supplier_tag.created",
        "supplier_tag.updated",
...
def test_router_assignment_is_idempotent():
    ...
    # must not raise on a second call with the same (supplier, tag).
