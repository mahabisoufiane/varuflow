...
ROUTER_SRC    = _read("app/routers/expense_budgets.py")
MIGRATION_SRC = _read("migrations/versions/c8d0e2f4a9b2_v97_expense_budgets.py")
MODEL_SRC     = _read("app/models/expense_budget.py")
...
def test_main_imports_router():
    assert "expense_budgets" in MAIN_SRC


def test_main_includes_router():
    assert "app.include_router(expense_budgets.router)" in MAIN_SRC
