...
export default function DashboardScreen() {
  ...
  const kpis: KPI[] = summary
    ? [
        { label: "Revenue (MTD)",  value: `${(summary.revenue / 1000).toFixed(1)}k kr`, change: "", up: true,  emoji: "💰" },
        { label: "Open Invoices",  value: String(summary.open_invoices),                 change: "", up: false, emoji: "🧾" },
    ...
    <SafeAreaView style={styles.safe}>
      {isTablet && (
        <TabletTopBar
          ...
          subtitle={new Date().toLocaleDateString("en-SE", {
            weekday: "long", month: "long", day: "numeric",
          })}
          testID="dashboard-tablet-topbar"
        />
      )}
      <ScrollView
      ...
      >
        {!isTablet && (
          /* Phone header — unchanged. Tablet uses TabletTopBar above. */
        ...
        {/* KPI grid — flex-wrap already lays out 2-up on phone;
            on tablet landscape we push card minWidth down to ~30% so
            3 fit per row without extra markup. */}
        <View
          testID={isTablet ? "dashboard-kpi-grid-tablet" : "dashboard-kpi-grid"}
          style={styles.kpiGrid}
        >
          {loading
            ? [0, 1, 2, 3].map((i) => <View key={i} style={[styles.kpiCard, styles.kpiSkeleton, isTablet && isLandscape && styles.kpiCardTabletLandscape]} />)
            : kpis.map((k) => (
                <View
                  key={k.label}
                  style={[styles.kpiCard, isTablet && isLandscape && styles.kpiCardTabletLandscape]}
                >
        ...
        {/* Tablet split-pane: low stock + recent activity side by side when
            landscape. Portrait stacks vertically (like phone). */}
        {isTablet && isLandscape ? (
          <View style={styles.tabletSplit} testID="dashboard-split">
            {(lowStock ?? []).length > 0 && (
              <View style={{ flex: 1 }}><LowStockAlert items={lowStock!} /></View>
            )}
            <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  kpiCardTabletLandscape: { minWidth: "30%" },
  ...
  tabletSplit: { flexDirection: "row", gap: 16, marginBottom: 16 },
});
